...
```cpp
// 주요 코드
float hue = map(ch3_pulse, 1000, 2000, 0, 270);  // 0~270도 색환 범위로 매핑
hsvToRgb(hue, 1.0, 1.0, r, g, b);                // HSV → RGB 변환 후 출력
```
🎯 0 ~ 270도 범위로 제한한 이유
색상환(Hue)은 360도로 구성되며, 360°와 0°는 같은 빨간색 계열

따라서 360°까지 매핑하면 색이 반복되어 이질적인 전환 구간 발생

본 프로젝트에서는 청록 ~ 빨강까지의 유효 스펙트럼(0~270°) 만을 사용하여 중복 없이 자연스러운 색상 전환 구현
